...
FInputData USQTEUtilityFunctionLibrary::GetInputParameters(float EndTime, bool bHasOpeningSeq, FInputStruct inputRef)
{
	...
	dataRef.bInstantFailure = inputRef.KeyData.bInstantFailure;
	...
	if (!inputRef.KeyData.bHasSequence)
	{
		...
		else
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("{GetInputParameters} KeyToPress key is not valid"));
		}

		UE_LOG(LogTemp, Warning, TEXT("{GetInputParameters} No sequence detected. Using regular key press only"));
	}
	else
	{
		dataRef.bHasSequence = true;

		UE_LOG(LogTemp, Warning, TEXT("{GetInputParameters} Sequence detected."));
		...
		if (!inputRef.KeyData.KeySequence.bTimed)
		{
			for (FKey Key : inputRef.KeyData.KeySequence.KeyCombos)
			{
				...
				{
					UE_LOG(LogTemp, Warning, TEXT("{GetInputParameters} Key data->KeySequence->KeyCombos one of the keys is not valid"));
					dataRef.bValidInput = false;
				}
			}

			UE_LOG(LogTemp, Warning, TEXT("{GetInputParameters} Sequence detected but no timed event, so using combo keys"));
		}
		else
		{
			dataRef.bTimed = true;
				...
				for (FKeyToTimeBindHelper KeyBinding : inputRef.KeyData.KeySequence.KeyTimeBind)
				{
					...
					else
					{
						UE_LOG(LogTemp, Warning,
							TEXT("{GetInputParameters} Something is wrong with the sequence binding for this reference. These keys are not valid in the data table. KeyData->KeySequence->KeyTimeBind. Explanation: %s"),
							*KeyBinding.FailedReasoning);

						dataRef.bValidInput = false;
					}
				}
				UE_LOG(LogTemp, Warning, TEXT("{GetInputParameters} Sequence detected and timed event detected. Using time bound sequence"));
			}
			else
			{
				dataRef.bValidInput = false;
				UE_LOG(LogTemp, Warning, TEXT("{GetInputParameters} Error, start times in all sequence bindings are not in order."));
			}
		}
	}
...
float USQTEUtilityFunctionLibrary::GetSequenceEndTime(ALevelSequenceActor* CurrentSeq)
{
	float Value = -1.0f;
	if (CurrentSeq)
	{
		Value = float(CurrentSeq->SequencePlayer.Get()->GetEndTime().AsSeconds());
	}
	return Value;
}
...
void USQTEUtilityFunctionLibrary::GetConditionalParameters(FConditionalQTE inRef, bool bHasOpening, FConditionalQTE& outRef, bool& Valid)
{
	...
	if (inRef.inputData.Data.Num() > 0)
	{
		for (int32 i = 0; i < inRef.inputData.Data.Num(); i++)
		{
			...
			{
				FConditionalQTEHelperData DataToAdd(inRef.inputData.Data[i].Keys, inRef.inputData.Data[i].Threshold);
				outRef.inputData.Data.Add(DataToAdd);

				if (!outRef.inputData.Data[i].Keys.IsValid())
				{
					Valid = false;
					UE_LOG(LogTemp, Warning, TEXT("{GetConditionalParameters} InputData->Data->Key choices aren't valid in conditional QTE key selection"));
					UE_LOG(LogTemp, Warning, TEXT("{GetConditionalParameters} %s"), *outRef.inputData.Data[i].Keys.FailExplanation);
					return;
				}
			}
			else if (inRef.inputData.Data[i].PreciseKeys.Num() > 0)
			{
				...
				for (FPrecisionKeys PrecKeys : outRef.inputData.Data[i].PreciseKeys)
				{
					...
					{
						Valid = false;
						UE_LOG(LogTemp, Warning, TEXT("{GetConditionalParameters} Precision keys aren't valid"));
						UE_LOG(LogTemp, Warning, TEXT("{GetConditionalParameters} %s"), *PrecKeys.FailExplanation);
						return;
					}
				}
			}
			...
			else if (inRef.inputData.Data[i].subSeq.KeyMap.Num() > 0)
			{
				...
				if (outRef.inputData.Data[i].subSeq.KeyMap.Num() > 0)
				{
					for (FSubSequenceHelper Helper : outRef.inputData.Data[i].subSeq.KeyMap)
					{
						...
						{
							Valid = false;
							UE_LOG(LogTemp, Warning, TEXT("{GetConditionalParameters} SubSequence map isn't valid"));
							UE_LOG(LogTemp, Warning, TEXT("{GetConditionalParameters} %s"), *Helper.FailExplanation);
							return;
						}
					}
				}
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("{GetConditionalParameters} Error. input is not a key, precision key nor subsequence or their values are empty"));
				Valid = false;
				return;
			}
			...
			if (l != inRef.inputData.Data.Num())
			{
				...
				{
					Valid = false;
					UE_LOG(LogTemp, Warning, TEXT("{GetConditionalParameters} Thresholds are not in order. One or more of the previous is higher than the following in the array"));

					UE_LOG(LogTemp, Warning, TEXT("{GetConditionalParameters} Threshold at index %s is greater than the threshold at index %s"), *FString::FromInt(i), *FString::FromInt(l));
					UE_LOG(LogTemp, Warning, TEXT("{GetConditionalParameters} Threshold at index '%s: %s'. Threshold at index '%s: %s'"),
						*FString::FromInt(i), *FString::FromInt(inRef.inputData.Data[i].Threshold),
						*FString::FromInt(l), *FString::FromInt(inRef.inputData.Data[l].Threshold));
	...
	{
		Valid = false;
		UE_LOG(LogTemp, Warning, TEXT("{GetConditionalParameters} InputData has no references entry. Please add one to the data table."));
		return;
	}
